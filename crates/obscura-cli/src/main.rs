    ...
    match args.command {
        ...
        Some(Command::Fetch { url, dump, selector, wait, timeout, wait_until, user_agent, eval, output, quiet, storage_dir, file, concurrency }) => {
            if let Some(file) = file {
                ...
                run_batch_fetch(urls, concurrency.get(), timeout, user_agent, global_proxy, output, quiet, stealth).await?;
            } else {
...
async fn fetch_original_bytes(
    ...
    Ok(fetch_original_response(url_str, proxy, user_agent, timeout_secs, stealth).await?.body)
}
